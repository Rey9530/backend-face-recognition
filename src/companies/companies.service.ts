...
export class CompaniesService {
  ...
  findAll() {
    return this.prisma.marca_empre_empresas.findMany({
      where: { empre_estado: 'ACTIVE' },
      orderBy: { empre_nombre: 'asc' },
    });
  }

  async findOne(id: string) {
    let register = await this.prisma.marca_empre_empresas.findFirst({
      where: { marca_empre_pk: id, empre_estado: 'ACTIVE' },
    });
